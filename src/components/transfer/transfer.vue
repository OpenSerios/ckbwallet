  ...
  <div class="transfer_card">
    ...
    <div class="card_body">
      <div v-if="assetArray.length!==0">
        ...
        <faucet-link v-if="faucetLink" class="faucet" />
      </div>
      <div v-else class="new_order_Form">
        <tx-list ref="txList" class="tx_list" @change="updateTxList" />
        <div>
          <div class="fees">
            <h4>{{ $t('transfer.fees') }}</h4>
            <p>{{ $t('transfer.fee_tx') }} <span>0.000000000 CKB</span></p>
          </div>
          <div class="advanced">
            <v-expansion-panels accordion class="advanced_panel" flat>
              <v-expansion-panel>
                ...
                <v-expansion-panel-content>
                  ...
                  <radio-buttons class="radio_buttons" :default_val="selectedAddress" :value="[selectedAddress]" @change="changeAddressesChange" />
          ...
          <div>
            ...
            <qr-input v-model="addressIn" class="qrIn" />
          </div>

          <div class="checkout">
            <ul v-if="errors.length>0" class="err_list">
              <li v-for="err in errors" :key="err">{{ err }}</li>
            </ul>
            <v-btn depressed class="but_primary" color="#2960CD" :loading="isAjax" :ripple="false" :disabled="!canSend" block @click="formCheck">{{ $t('transfer.send') }}</v-btn>
          </div>
        </div>
      </div>
    </div>
...
import RadioButtons from '@/components/modals/RadioButtons'
import QrInput from '@/components/common/QrInput'
import FaucetLink from '@/components/modals/FaucetLink'
...
export default {
  ...
  computed: {
    faucetLink() {
      const link = process.env.VUE_APP_FAUCET_LINK
      if (link) return link
      return null
    },
    canSend() {
